...
Additional less commonly used commands:
  ...
  chainstate [ARGS] Run drip kernel chainstate util, equivalent to running 'bitcoin-chainstate [ARGS]'.
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            // Since "drip rpc" is a new interface that doesn't need to be
...
//!
//!     strace -e trace=execve -s 10000 build/bin/drip ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/drip/drip/pull/31375#discussion_r1861814807)
