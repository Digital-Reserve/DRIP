...
enum class FeeEstimateMode {
    ...
    BTC_KVB,      //!< Use DRIP/kvB fee rate unit
    SAT_VB,       //!< Use drip/vB fee rate unit
};

/**
 * Fee rate in drips per virtualbyte: CAmount / vB
 * the feerate is represented internally as FeeFrac
 */
class CFeeRate
{
private:
    /** Fee rate in sats/vB (drips per N virtualbytes) */
    FeePerVSize m_feerate;

public:
    /** Fee rate of 0 drips per 0 vB */
    CFeeRate() = default;
    ...
    /**
     * Construct a fee rate from a fee in drips and a vsize in vB.
     *
    ...
    /**
     * Return the fee in drips for the given vsize in vbytes.
     * If the calculated fee would have fractional drips, then the
     * returned fee will always be rounded up to the nearest drip.
     */
    ...
    /**
     * Return the fee in drips for a vsize of 1000 vbytes
     */
