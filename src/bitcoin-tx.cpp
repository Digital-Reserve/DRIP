...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The bitcoin-tx tool is used for creating and modifying drip transactions.\n\n"
                "bitcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded drip transaction, or with \"-create [commands]\" to create a hex-encoded drip transaction.\n"
                "\n"
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded drip transaction
