...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of drops so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    BitcoinUnit nDisplayUnit = BitcoinUnit::DRIP;
    ...
    // how many drops the estimated fee can vary per byte we guess wrong
    double dFeeVary = (nBytes != 0) ? (double)nPayFee / nBytes : 0;

    QString toolTip4 = tr("Can vary +/- %1 drip(s) per input.").arg(dFeeVary);
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show drip address. In tree mode, address is not shown again for direct wallet address outputs
