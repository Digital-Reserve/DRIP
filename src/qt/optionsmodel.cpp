...
/** Call node.updateRwSetting() with DRIP 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because DRIP 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/drip/drip/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because drip 22.x will not try to read these.
...
//! Convert enabled/size values to drip -prune setting.
...
//! Get pruning enabled value to show in GUI from drip -prune setting.
...
//! Get pruning size value to show in GUI from drip -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayBitcoinUnit")) {
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(BitcoinUnit::DRIP));
    }
    QVariant unit = settings.value("DisplayBitcoinUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bitcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bitcoin_unit = BitcoinUnit::DRIP;
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/drip/drip/pull/8273
