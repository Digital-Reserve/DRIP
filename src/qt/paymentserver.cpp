...
const QString BITCOIN_IPC_PREFIX("drip:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // drip: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click drip: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start drip: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling drip: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("drip://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'drip://' is not a valid URI. Use 'drip:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // drip: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid DRIP address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
