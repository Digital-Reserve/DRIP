...
public:
    CMainParams() {
        ...
        m_assumed_blockchain_size = 1;  // DRIP: New blockchain starts small
        ...
        vSeeds.emplace_back("seed.drip.petertodd.net."); // Peter Todd, only supports x1, x5, x9, and xd
...
/**
 * DRIP network: Decentralized Reset for Investing People.
 * - 5 minute block time
 * - 21 million supply
 * - No premine
 * - CPU-friendly mining initially (low difficulty)
 */
class CDripParams : public CChainParams {
public:
    CDripParams() {
        m_chain_type = ChainType::DRIP;
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        ...
        // Message start: DRIP pattern (0x44 0x52 0x49 0x50)
        ...
        // IMPORTANT: Genesis block funds are unspendable (like DRIP)
        // The genesis block transactions are skipped during ConnectBlock(),
        // so outputs are never added to the UTXO set (validation.cpp:2378-2384)
        // This ensures genesis block coinbase cannot be spent and won't appear in wallets
        ...
        genesis = CreateGenesisBlock(drip_genesis_msg,
                ...
                nBits,
                1,
                50 * COIN); // 50 coins per block, no premine
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        // No DNS seeds - using HTTP seeds for peer discovery
        ...
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 0x1F); // gives 'E' for script addresses
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1, 0xD2); // Private key prefix
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};
...
std::unique_ptr<const CChainParams> CChainParams::Drip()
{
    return std::make_unique<const CDripParams>();
}
...
std::optional<ChainType> GetNetworkForMagic(const MessageStartChars& message)
{
    ...
    const auto drip_msg = CChainParams::Drip()->MessageStart();
    ...
    } else if (std::ranges::equal(message, drip_msg)) {
        return ChainType::DRIP;
    }
    return std::nullopt;
}
