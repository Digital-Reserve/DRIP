// Copyright (c) 2020-present The DRIP developers
...
FUZZ_TARGET(pow, .init = initialize_pow)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.remaining_bytes() > 0, 10000) {
        ...
        }
        {
            ...
            (void)CalculateNextWorkRequired(&current_block, fuzzed_data_provider.ConsumeIntegralInRange<int64_t>(0, std::numeric_limits<int64_t>::max()), consensus_params, current_block.nHeight + 1);
