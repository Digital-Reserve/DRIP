...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    // This tx has a low fee: 1000 drips
    Txid hashParentTx = tx.GetHash(); // save this txid for later use
    const auto parent_tx{entry.Fee(1000).Time(Now<NodeSeconds>()).SpendsCoinbase(true).FromTx(tx)};
    TryAddToMempool(tx_mempool, parent_tx);

    // This tx has a medium fee: 10000 drips
    ...
    tx.vout[0].nValue = 5000000000LL - 1000 - 50000; // 50k drip fee
    ...
    // waitNext() should return if fees for the new template are at least 1 drip up
    ...
    tx.vout[0].nValue = 100000000 - 10000; // 10k drip fee
...
void MinerTestingSetup::TestPrioritisedMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vin[0].prevout.hash = hashParentTx;
    tx.vout[0].nValue = 5000000000LL - 1000 - 1000; // 1000 drip fee
