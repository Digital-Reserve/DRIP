// Copyright (c) 2010 Drip Nakamoto
...
static bool GenerateBlock(ChainstateManager& chainman, CBlock&& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    const bool infinite_mining = (max_tries == 0);
    while ((infinite_mining || max_tries > 0) && !CheckProofOfWork(block.GetHash(), block.nBits, chainman.GetConsensus()) && !chainman.m_interrupt) {
        ++block.nNonce;
        if (!infinite_mining) {
            --max_tries;
        }
        ...
        // If nonce wraps, update timestamp and reset nonce
        if (block.nNonce == 0) { // Wrapped around
            int64_t nNewTime = std::max<int64_t>(pindexPrev ? pindexPrev->GetMedianTimePast() + 1 : 0,
                                                 TicksSinceEpoch<std::chrono::seconds>(NodeClock::now()));
            if (nNewTime > block.nTime) {
                block.nTime = nNewTime;
                block.hashMerkleRoot = BlockMerkleRoot(block); // Recalculate merkle root if time changed
                LogInfo("Mining: updated timestamp to %d, recalculated merkle root\n", block.nTime);
            } else {
                // Can't update timestamp, need new template
                LogInfo("Mining: nonce wrapped, timestamp can't be updated, need new template\n");
                return false;
            }
        }
        
        // Update timestamp periodically even if nonce hasn't wrapped
        else if (tries_done % TIMESTAMP_UPDATE_INTERVAL == 0) {
            int64_t nNewTime = std::max<int64_t>(pindexPrev ? pindexPrev->GetMedianTimePast() + 1 : 0,
                                                 TicksSinceEpoch<std::chrono::seconds>(NodeClock::now()));
            if (nNewTime > block.nTime) {
                block.nTime = nNewTime;
                block.hashMerkleRoot = BlockMerkleRoot(block);
                LogInfo("Mining: updated timestamp to %d at %llu tries, recalculated merkle root\n", block.nTime, tries_done);
            } else {
                // Force timestamp update if we've tried many nonces without success
                if (tries_done >= 50000000) {
                    block.nTime = TicksSinceEpoch<std::chrono::seconds>(NodeClock::now());
                    block.hashMerkleRoot = BlockMerkleRoot(block);
                    LogInfo("Mining: forced timestamp update to %d at %llu tries\n", block.nTime, tries_done);
                }
            }
        }
        
        // Log progress every 1M tries for debugging
        if (tries_done % 1000000 == 0) {
            LogInfo("Mining: tried %llu nonces (current: %u, target: %s)\n", 
                     tries_done, block.nNonce, block.GetHash().ToString());
        }
    }
    
    if (chainman.m_interrupt) {
        LogInfo("Mining: interrupted after %llu tries, nonce range %u-%u, final hash: %s\n",
                 tries_done, start_nonce, block.nNonce, block.GetHash().ToString());
        return false;
    }
    if (!infinite_mining && max_tries == 0) {
        LogInfo("Mining: exhausted %llu tries, nonce range %u-%u, final hash: %s\n",
                 tries_done, start_nonce, block.nNonce, block.GetHash().ToString());
        return false;
    }
    if (block.nNonce == std::numeric_limits<uint32_t>::max()) {
        LogInfo("Mining: nonce wrapped around, need new template\n");
        return false; // Changed from true - nonce wrap means we need a new template
    }
    
    LogInfo("Mining: found block after %llu tries, nonce: %u, hash: %s\n",
             tries_done, block.nNonce, block.GetHash().ToString());
...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "Mine to a specified descriptor and return the block hashes.",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated drip to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const auto max_tries_param{self.Arg<uint64_t>("maxtries")};
    const uint64_t max_tries = (max_tries_param == 0) ? 0 : max_tries_param; // 0 = infinite mining
        ...
        "Mine to a specified address and return the block hashes.",
         {
             ...
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated drip to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{0}, "How many iterations to try (0 = mine infinitely until block found)."},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
             {
         ...
         RPCExamples{
            ...
            + "If you are using the " CLIENT_NAME " wallet, you can get a new address to send the newly generated drip to with:\n"
            + HelpExampleCli("getnewaddress", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const int num_blocks{request.params[0].getInt<int>()};
    const uint64_t max_tries{request.params[2].isNull() ? 0 : static_cast<uint64_t>(request.params[2].getInt<int64_t>())}; // 0 = infinite mining
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated drip to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::shared_ptr<const CBlock> block_out;
    uint64_t max_tries{0}; // 0 = infinite mining
...
// NOTE: Unlike wallet RPC (which use DRIP values), mining RPCs follow GBT (BIP 22) in using drip amounts
static RPCHelpMan prioritisetransaction()
{
    return RPCHelpMan{"prioritisetransaction",
            ...
            "                  DEPRECATED. For forward compatibility use named arguments and omit this parameter."},
                    {"fee_delta", RPCArg::Type::NUM, RPCArg::Optional::NO, "The fee value (in drips) to add (or subtract, if negative).\n"
        ...
        RPCResult{
            RPCResult::Type::OBJ_DYN, "", "prioritisation keyed by txid",
            {
                {RPCResult::Type::OBJ, "<transactionid>", "", {
                    {RPCResult::Type::NUM, "fee_delta", "transaction fee delta in drips"},
                    {RPCResult::Type::BOOL, "in_mempool", "whether this transaction is currently in mempool"},
                    {RPCResult::Type::NUM, "modified_fee", /*optional=*/true, "modified fee in drips. Only returned if in_mempool=true"},
                }}
            },
        },
        RPCExamples{
...
static RPCHelpMan getblocktemplate()
{
    return RPCHelpMan{
        ...
        "    https://github.com/drip/bips/blob/master/bip-0022.mediawiki\n"
        "    https://github.com/drip/bips/blob/master/bip-0023.mediawiki\n"
        "    https://github.com/drip/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
        "    https://github.com/drip/bips/blob/master/bip-0145.mediawiki\n",
        {
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::ARR, "transactions", "contents of non-coinbase transactions that should be included in the next block",
                {
                    {RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "fee", "difference in value between transaction inputs and outputs (in drips); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one"},
                ...
                {RPCResult::Type::NUM, "coinbasevalue", "maximum allowable input to coinbase transaction, including the generation award and transaction fees (in drips)"},
