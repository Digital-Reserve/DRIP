...
const char * const BITCOIN_CONF_FILENAME = "drip.conf";
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ...
        ChainTypeToString(ChainType::DRIP),
    };
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\DRIP
    //   new: C:\Users\Username\AppData\Local\DRIP
    // macOS: ~/Library/Application Support/DRIP
    // Unix-like: ~/.drip
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "DRIP";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "DRIP";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/DRIP";
#else
    // Unix-like
    return pathRet / ".drip";
#endif
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fDrip = get_net("-drip");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fTestNet + (int)fTestNet4 + (int)fDrip > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -signet, -testnet, -testnet4, -drip and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fDrip) return ChainType::DRIP;
    // Default to DRIP instead of Bitcoin MAIN
