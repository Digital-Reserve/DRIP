// Copyright (c) 2024-present The DRIP developers
...
BOOST_AUTO_TEST_CASE(bnb_test)
{
    ...
    for (int feerate : feerates) {
        ...
        // BnB fails to find changeless solution when overshooting by cost_of_change + 1 drip
...
BOOST_AUTO_TEST_CASE(bnb_feerate_sensitivity_test)
{
    ...
    std::vector<OutputGroup> low_feerate_pool; // 5 drip/vB (default, and lower than long_term_feerate of 10 drip/vB)
    ...
    std::vector<OutputGroup> high_feerate_pool; // 25 drip/vB (greater than long_term_feerate of 10 drip/vB)
