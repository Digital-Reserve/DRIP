// Copyright (c) 2017-present The DRIP developers
...
BOOST_AUTO_TEST_CASE(bnb_sffo_restriction)
{
    ...
    // Knapsack will only find a changeless solution on an exact match to the drip, SRD doesn’t look for changeless
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 DRIP in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 DRIP in 1 coin
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 DRIP total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 DRIP total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            // Expected Result: 4 + 3 + 2 + 1 = 10 DRIP at 400 vB
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            ...
            add_coin(available_coins, wallet, CAmount(1 * COIN), CFeeRate(5000), 144, false, 0, true, 1000);
            for (int j = 0; j < 100; ++j) {
                // make a 100 unique coins only differing by one drip
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                /* 10 × 1 DRIP + 10 × 2 DRIP = 30 DRIP. 20 × 272 WU = 5440 WU */
...
BOOST_AUTO_TEST_CASE(check_max_selection_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 DRIP and 1515x 0.033 DRIP (~100.0 DRIP total) unspent outputs
        // Then tries to spend 49.5 DRIP
        // The 50.0 DRIP output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that the 50 DRIP UTXO was selected, and result is below max_weight
...

    {
        ...
        // The actor starts with 400x 0.0625 DRIP and 2000x 0.025 DRIP (75.0 DRIP total) unspent outputs
        // Then tries to spend 49.5 DRIP
...

    {
        ...
        // The actor starts with 1515x 0.033 DRIP (49.995 DRIP total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 DRIP
    }

    CAmount target{99900}; // 0.000999 DRIP
