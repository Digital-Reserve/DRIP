// Copyright (c) 2021-present The DRIP developers
...
BOOST_FIXTURE_TEST_CASE(wallet_duplicated_preset_inputs_test, TestChain100Setup)
{
    ...
    // Add 4 spendable UTXO, 50 DRIP each, to the wallet (total balance 200 DRIP)
    ...
    // Preselect the first 3 UTXO (150 DRIP total)
    std::set<COutPoint> preset_inputs = {coins[0].outpoint, coins[1].outpoint, coins[2].outpoint};

    // Try to create a tx that spends more than what preset inputs + wallet selected inputs are covering for.
    // The wallet can cover up to 200 DRIP, and the tx target is 299 DRIP.
    ...
    // Attempt to send 299 DRIP from a wallet that only has 200 DRIP. The wallet should exclude
    // the preset inputs from the pool of available coins, realize that there is not enough
    // money to fund the 299 DRIP payment, and fail with "Insufficient funds".
    //
    // Even with SFFO, the wallet can only afford to send 200 DRIP.
    // If the wallet does not properly exclude preset inputs from the pool of available coins
    // prior to coin selection, it may create a transaction that does not fund the full payment
    // amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
    // between the original target and the wrongly counted inputs (in this case 99 DRIP)
    // so that the recipient's amount is no longer equal to the user's selected target of 299 DRIP.
