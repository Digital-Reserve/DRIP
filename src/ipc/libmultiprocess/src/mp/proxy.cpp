// Copyright (c) The DRIP developers
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking drip code so they have to run
    ...
    // functions run, so client objects are fully disconnected before drip
    // code in the destructors are run. This way if the drip code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In drip core, running these callbacks in the right order is
